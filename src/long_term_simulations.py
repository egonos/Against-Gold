...
def simulate_twenty_years_of_investment(
    df, purchase_times=10, sample_size=1, etf_per_purchase=2, expense_ratio=0.00095
):
    ...
    for start_year, end_year in tqdm(investment_periods):
        ...
        for i in range(sample_size):
            buy_prices = (
                start.sample(purchase_times, replace=False, random_state=i)[
                    "Close"
                ].values
                / 10
            )
            ...
            for year in range(start_year + 1, end_year + 1):
                ...
                annual_growth_constant = (
                    df[df["Year"] == year]["Close"].mean()
                    / df[df["Year"] == year - 1]["Close"].mean()
                )
                annual_return_with_divs = annual_growth_constant + divs[year - 1] / 100
                annual_return_with_divs_expenses = (
                    annual_return_with_divs - expense_ratio
                )
                annual_return_without_divs_expenses = (
                    annual_growth_constant - expense_ratio
                )
            ...
            real_returns.append(
                (
                    ...
                    capital_invested,
                    portfolio_value,
                    portfolio_value - capital_invested,
                    capital_invested_adjusted,
                    portfolio_value_adjusted,
                    percentage_change_not_invested,
                    percent_change,
                    (portfolio_value_adjusted_not_invested - capital_invested_adjusted),
                )
...
def simulate_twenty_years_of_investment_gold(
    df, sample_size=30, purchase_times=10, ounce_per_purchase=2
):
    investment_periods = [(i, i + 20) for i in range(1950, 2004)]
    real_returns = []
    for start_year, end_year in tqdm(investment_periods):
        ...
        for i in range(sample_size):
            buy_prices = start.sample(purchase_times, replace=False, random_state=i)[
                "Close"
            ].values
