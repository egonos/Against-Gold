...
def display_EMA(df, start=100, end=200):
    ...
    EMA = [
        mpf.make_addplot(
            df.iloc[start:end, :]["EMA 3"], color="darkorange", label="EMA 3"
        ),
    ...
    plt.figure()
    mpf.plot(
        df.iloc[100:200, :], type="candle", volume=False, style="charles", addplot=EMA
    )
    plt.show()
    plt.rcdefaults()
...
def simulate_control_group(df, etf_purchased=20, expense_rate=0.00095):
    ...
    results = pd.DataFrame(
        ...
        columns=[
            ...
            "Expenses",
            "Capital Gained",
            "(%)Annual_Return_Without_Dividends",
        ],
    ).set_index("Period")
...
def simulate_trade_EMA(
    df, etf_purchased=20, expense_rate=0.00095, EMA1=12, EMA2=26, verbose=False
):
    """
    ...
    # buy-sell signals
    df["Buy"] = (df[f"EMA{str(EMA1)}"] > df[f"EMA{str(EMA2)}"]) & (
        df[f"EMA{str(EMA1)}"].shift() <= df[f"EMA{str(EMA2)}"].shift()
    )
    df["Sell"] = (df[f"EMA{str(EMA1)}"] < df[f"EMA{str(EMA2)}"]) & (
        df[f"EMA{str(EMA1)}"].shift() >= df[f"EMA{str(EMA2)}"].shift()
    )
    ...
    for year in trade_periods:
        ...
        for index, row in df_trade.iterrows():
            if not position_condition and row["Buy"]:
                ...
                if cash_balance == 0:
                    ...
                    if verbose:
                        ...
                        print(
                            "Capital invested to this trade:",
                            etfs_holding * row["ETF Price"],
                        )
            ...
            # sell condition
                ...
                cash_balance -= (
                    etfs_holding * row["ETF Price"] * expense_rate * days_held / 365
                )
                total_expenses += (
                    etfs_holding * row["ETF Price"] * expense_rate * days_held / 365
                )
        ...
        if position_condition:
            ...
            if verbose:
                print(
                    ...
                    df_trade.iloc[-1]["ETF Price"],
                    "\nCapital gained from trade:",
                    etfs_holding * df_trade.iloc[-1]["ETF Price"],
                    "\nExpense payment:",
                    etfs_holding
                    * df_trade.iloc[-1]["ETF Price"]
                    * expense_rate
                ...
                if cash_balance == 0:
                    ...
                    if verbose:
                        ...
                        print(
                            "Bought",
                            ounce_purhcased,
                            "ounces of gold at a price of: ",
                            row["Close"],
                        )
                        print(
                            "Capital invested to this trade:",
                            etfs_holding * row["Close"],
                        )
                        print("Cash in hand:", cash_balance)
                        print()
        ...
        if position_condition:
            ...
            if verbose:
                print(
                    ...
                    etfs_holding,
                    "ounces of gold at a price of:",
                    df_trade.iloc[-1]["Close"],
                    "\nCapital gained from trade:",
                    etfs_holding * df_trade.iloc[-1]["Close"],
                    "\nNet cash:",
                    cash_balance,
                )
            etfs_holding = 0
