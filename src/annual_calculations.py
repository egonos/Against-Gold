...
def compute_annual_returns_stocks(df):
    ...
    for year in tqdm(investment_periods):
        ...
        adjusted_annual_return_without_dividends = (
            (current_year_mean - adjusted_previous_mean) / adjusted_previous_mean * 100
        )
        adjusted_annual_return_with_dividends = (
            (current_year_mean + adjusted_dividend_return - adjusted_previous_mean)
            / adjusted_previous_mean
            * 100
        )
...
def compute_annual_returns_stocks_individually_display(df):
    ...
    individual_return_display_df = pd.DataFrame(
        data=annual_returns, columns=["Period", "Month", "(%)Adjusted_Real_Returns"]
    ).set_index("Period")
...
def compute_annual_returns_commodity(df, start=1951, end=2024):
    ...
    results = pd.DataFrame(
        annual_returns, columns=["Period", "(%)Adjusted_Annual_Return"]
    )
    results = results.set_index("Period")
    return results
...
def compute_annual_returns_gold_individually(df):
    ...
    for year in tqdm(range(1951, 2024)):
        ...
        for month in range(1, 13):
            ...
            adjusted_previous_mean = previous_year_mean * inflation_constant
            adjusted_annual_return = (
                (current_year_mean - adjusted_previous_mean)
                / adjusted_previous_mean
                * 100
            )
